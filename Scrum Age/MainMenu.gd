...
func _on_Start_pressed():
	if _checkPlayers() == false:
		get_node("WarningPanel").visible = true;
		return;
...
func _on_Button_pressed():
	get_node("WarningPanel").visible = false;
	
func _checkPlayers():
	var readyToPlay = false;
	if $VBoxContainer/VBoxContainer/PlayerOne.text == "Player One" || $VBoxContainer/VBoxContainer/PlayerTwo.text == "Player Two":
		return readyToPlay
